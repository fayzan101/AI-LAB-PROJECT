...
_TELEMETRY_EXTRA_KEYS = (
    "active_seconds",
    "idle_seconds",
    "telemetry_days_with_data",
    "segment_count",
    "focus_fragmentation_index",
    "first_seen_offset_minutes",
    "last_seen_offset_minutes",
)


def _employee_extra_json(payload: dict[str, Any]) -> str:
    extra = {k: payload[k] for k in _TELEMETRY_EXTRA_KEYS if k in payload and payload[k] is not None}
        ...
        row = conn.execute(
            text(
                ...
                INSERT INTO employee_inputs (
                    ...
                    task_progress, days_left, late_arrivals, absent_days, extra_json
                )
                VALUES (
                    :tenant_id, :employee_id, :tasks_completed, :attendance_days, :idle_hours, :working_hours,
                    :task_progress, :days_left, :late_arrivals, :absent_days, :extra_json
