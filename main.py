...
- **Do not call from browsers.** The portal **Node backend** authenticates with `POST {prefix}/auth/login` using `client_id` / `client_secret` / `tenant_id`, then proxies employee sync, analytics, tasks, and reports.
- Portal routes that wrap this API live under **`/api/v1/ai/*`** (see portal `openapi.json` at `/docs` or `public/swagger/openapi.json`).
- Default API prefix for **this** service: **`{prefix}`** (`API_PREFIX`).
""".format(prefix=settings.api_prefix).strip()
...
app = FastAPI(
    ...
    openapi_tags=[
        {"name": "Authentication", "description": "Service-to-service JWT (portal-backend → ai-server)."},
        {"name": "Employees", "description": "Ingest employee snapshots (incl. optional agent telemetry scalars)."},
        {"name": "Tasks", "description": "Ingest task progress updates."},
        {"name": "Analytics", "description": "Synchronous analytics / full report generation."},
        {"name": "Reports", "description": "Historical weekly report retrieval."},
